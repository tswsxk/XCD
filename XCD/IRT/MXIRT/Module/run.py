...
def numerical_check(_net, _cfg: Configuration, train_data, test_data, dump_result=False,
    ...
    loss_f = get_loss(**_cfg.loss_params)
    ...
    loss_monitor = MovingLoss(loss_f)
...
def train(train_fn, test_fn, reporthook=None, final_reporthook=None,
          primary_key="macro_auc", params_save=False, **cfg_kwargs):  # pragma: no cover
    ...
    net_init(_net, cfg=_cfg, **_cfg.init_params,
             int_df=train_df, user_num=_cfg.hyper_params["user_num"], item_num=_cfg.hyper_params["item_num"],
             logger=_cfg.logger
             )
